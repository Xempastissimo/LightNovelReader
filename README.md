...
- [x] 翻页模式（左右翻页 / 上下滚动，见 §3、§10）
- [x] 本地书签（仅限已整本下载的小说，见 §3、§10）
- [x] OLED 纯黑模式（长按「深色」/「夜间」，见 §3、§10）
- [ ] 轻小说排行榜下拉加载
- [ ] 简繁切换
- [ ] 自动更新
- [ ] ……
...
- 书架：继续阅读（本地阅读记录）/ 在线书架 / **已缓存**（本机已下载章节）/ **已下载**（站点打包的整本）；同步结果与补全提示显示在页面底部；删除本机下载时会提示该书的本地书签也一并删除
- 阅读器：翻页阅读 + 目录跳转 + 阅读设置面板 + 章节切换横幅提示 + 音量键翻页（含反转）；**两种翻页方向**——「左右翻页」把本章按屏幕高度**真实测量分页**（一页正好一屏，页内不再滚动），「上下滚动」是一整章连续滚动、不预先分页；两种模式都**预加载相邻章节**（下一章总是取，上一章只在本章开头取），翻到下一章不用等；**本地书签**（仅限已整本下载的小说，可添加 / 跳转 / 删除，删除该下载时书签一并删除）；**长按「夜间」切换 OLED 纯黑**（#000000）；**系统状态栏跟随顶底栏**——单点唤出顶底栏时状态栏一起出现，顶底栏收起（再点一次或 5 秒后自动收起）时一起隐藏，退出阅读时恢复
- 设置：账号管理、阅读外观（顶底栏配色、音量键翻页、**翻页方向**）、深色模式（**长按「深色」切换 OLED 纯黑**）、存储管理（离线章节 / 整本下载 / 图片缓存）、书源说明、开发者页（跳转项目 GitHub 主页）
...
│  └─ repo/                        对界面暴露的仓库层
│     ├─ BookRepository.kt         详情缓存 + 章节离线读写 + 逐章缓存 + 整本下载编排 + 删除本机副本
│     ├─ PackStore.kt              整本打包文件与索引（`filesDir/packs/`，见 §8.2）
│     ├─ ShelfRepository.kt        本地书架 / 阅读进度 / 搜索历史 / 元信息合并
│     ├─ BookmarkStore.kt          本地书签（`filesDir/library/bookmarks.json`，见 §10）
...
└─ ui/
   ...
   └─ screen/
      ...
      └─ reader/                   阅读器：两种翻页方向 + 目录 + 阅读设置面板 + 书签面板 + 章节切换横幅提示
         ├─ ReaderScreen.kt        有状态外壳 + 视口测量分页（`TextMeasurer`）+ 左右/上下两种排版
         ├─ ReaderPagination.kt    分页与定位的纯函数（按行装箱、段落锚点换算），可在 JVM 上测试
         ├─ ReaderPrefetch.kt      相邻章节预取的判定（纯函数）
         ├─ ChapterTurn.kt         越界滑动是否翻章（纯函数）
         └─ OverlayAutoHideTimer.kt 顶底栏自动收起计时（纯函数外壳）
```
...
| `ui/theme/ThemeOledTest` | `withOledBlack()` 把 `background`/`surface` 置为纯黑；`onBackground`/`onSurface`/`onSurfaceVariant` 与强调色不变；`surfaceContainer*` **保留原有抬升**（否则黑色菜单在黑色页面上没有边界）；对应用自带的深色方案同样生效 |
...
| 限制 | 说明与后续方向 |
|---|---|
| **分页按真实测量** | 左右翻页用 `TextMeasurer` 量出每段在**当前宽度**下的换行位置与真实高度（含字体 padding），再按行装箱成页，所以一页正好一屏、页内不再滚动。旧实现是「每页约 520 字」的字符预算，而唯一能修正它的 `updatePageCapacity()` 从未被调用，`ReaderPage` 又带 `verticalScroll`，于是超出的部分变成页内滚动——这正是本次修掉的问题。测量发生在 Compose 层（`ReaderScreen`），分页规则本身是纯函数（`ReaderPagination.kt`），不依赖设备即可断言「没有一页比屏幕高」。代价是开章时要量一遍全章（约 200～500 次 measure，通常十几毫秒），期间显示「正在排版…」；拖动字号滑块会重新测量，滑块本身被面板遮住，抖动不可见。 |
| **OLED 纯黑是隐藏手势** | 「长按深色 / 夜间」是一个看不见的操作，所以两处都在主题那一行下面写了说明行并显示当前状态（开启时用主题色），设置页还会弹一条 snackbar。**长按在打开它的同时也会选中对应的深色主题**：纯黑属于深色背景，在浅色主题下打开它只会置一个屏幕上什么都看不出来的标志位，而长按的本来就是写着「深色」的那个按钮。关闭时不动主题。两处（设置页的 `ChoiceRow`、阅读面板的 `ReaderChoiceChip`）都没有用 `OutlinedButton`/`TextButton`，而是 `Surface` + `combinedClickable`——按钮自己拥有点击处理，在它上面再叠一层 `pointerInput` 长按会让长按**同时触发按钮的点击**，触发顺序取决于指针分发而不是这个文件的决定。 |
| **OLED 只动两个颜色角色** | `withOledBlack()` 只改 `background` 与 `surface`：这两个是铺满整屏的角色（`Scaffold` 的容器、独立的 `Surface`），省电省在这里，而且改它们不会牺牲可读性。`surfaceContainer*` **故意保留抬升**——下拉菜单、底部弹层、导航栏都画在这些角色上，而纯黑页面上阴影是不可见的，把它们也涂黑等于让菜单没有边界。所以「纯黑」指的是页面底色，菜单仍是略浅的深色。 |
| **OLED 只在深色下有意义** | 浅色主题（应用浅色 / 米黄 / 白纸 / 护眼）不受影响：纯黑底配深色字等于看不见。因此这个开关在浅色下只是被记住，等切到深色才生效——说明行会一直显示当前状态。 |
| **页内不写死换行点** | 切分出来的段落碎片是**纯字符区间**，不插入 `\n`：`TextAlign.Justify` 是**均衡**断行，量出来的某一行可能离右边距还差很远（一行只有一个字也是它的正常结果），把这些断点写回文本就变成了硬换行——第一版正是这么写的，于是页面复刻了均衡排版的锯齿，出现「一个字一行」的孤儿行。改成让页面自己**贪心**重排：贪心断行需要的行数**不会多于**均衡断行，所以一页只会比装箱时算的**更短**，永远不会更高，溢出保证仍然成立。测量与绘制共用同一个 `TextStyle` 对象（`pageTextStyle`），避免两者对宽度有任何分歧。 |
| **阅读进度以段落为锚点** | `ReadingProgress.paragraphIndex` 存的是段落在 `ChapterContent.paragraphs` 中的下标，不是页码：页码随字号与窗口变化，段落不会。**这是语义变更**——旧存档里这个字段其实是页码，升级后首次恢复某一章会落在章内更靠前的位置（越界会被收敛到 0），翻一页即被新值覆盖，之后不再偏移。不做迁移代码：页码无法可靠还原成段落号，任何猜测都会变成更难解释的随机跳转。 |
| **本地书签仅限已整本下载的小说** | 书签只对「已下载」页签里的书开放（`filesDir/library/bookmarks.json`）。没整本下载的书点「书签」会看到原因说明，而不是一个空面板。**书签随下载一起删除**：`BookRepository.deleteLocalCopy` 是删本机副本的唯一入口，书签在那里一并清掉，所以「已下载」页签的删除、详情页的「删除本机下载」和设置页的「清理整本下载」三处都覆盖到了，删除确认框会写明将删掉几条。设置页「清理离线章节」**不**删书签：打包还在，那本书仍然是「已下载」。 |
| **上下滚动模式没有越界翻章手势** | 左右翻页可以「滑过本章末尾」直接进下一章（`resolveChapterTurn`）；上下滚动是一整章连续滚动，竖直拖动归列表所有，横向拖动不做任何事以免和滚动抢手势，因此改为在本章正文的**首尾各放一个「上一章 / 下一章」按钮**。音量键在两种模式都能用：左右翻页翻页，上下滚动按一屏滚动，到边界才翻章。 |
...
**用户真机阅读时发现、本轮已修的两个 bug**：

| 反馈 | 原因与修法 |
|---|---|
| 左右翻页时排版错乱：截图里「开」「【LoD】」各自占一行，行尾参差不齐 | 段落碎片一度用 `\n` 把**测量出来的换行点写死**。但 `TextAlign.Justify` 是**均衡**断行，量出的某一行可能离版心还差很远（一行只有一个字也是它的正常结果），写回文本后就变成了硬换行——页面于是复刻了均衡排版的锯齿。改为碎片只取**纯字符区间**、由页面自己贪心重排；贪心所需行数不多于均衡，所以一页只会更短、不会更高，溢出保证不受影响。回归测试：`fragments carry no line breaks of their own`、`a short measured line does not become a line of its own`；另外测量与绘制现在共用同一个 `TextStyle` 对象。 |
| 上下滚动模式下顶底栏只在进入时出现，之后怎么点都不出来 | 竖向排版是 `LazyColumn`，而点击回调只挂在横向的 `ReaderPage` / `IllustrationPage` 上，竖向的散文段落**根本没有点击处理**，`toggleMenu` 从未被调用。已把点击挂到列表容器上（边距与段间空隙同样可点）。 |

**这两个 bug 都是真机阅读才暴露、编译与单测没能拦住的**：一个是「测量出的换行点不该写回文本」这层语义（单测覆盖了装箱算术，覆盖不到渲染后的观感），一个是「同一个交互在两种排版下要各自接上」的遗漏。回归测试已补，但**下面这份设备清单仍需要再跑一遍**。

**尚未在真机/模拟器上复核**（本次开发环境没有可用设备，以下条目是按验收标准留给设备复核的清单，**不是**已验证结论）：

- [ ] A1：多种字号（12/18/24/32 sp）下逐页截图，确认正文底边都在屏幕内、**行尾整齐无孤儿行**、页面不再滚动；顶栏页码数与实际页数一致
- [ ] A2：切到「上下滚动」后是一整章连续滚动、顶栏显示「已读 NN%」、首尾出现「上一章 / 下一章」按钮；**点击正文任意位置（含左右边距、段间空隙）都能唤出顶底栏**，且上下滚动不受影响
- [ ] A3：设置页与阅读设置面板里切换翻页方向，阅读位置不丢（按段落锚定）
- [ ] A4：读到第 N 章后 `adb shell run-as com.xempastissimo.lightnovelreader ls files/library/{bookId}/` 能看到 `{N+1}.json`，翻到下一章没有加载态
- [ ] A5：断网后翻章，预加载失败不在界面上产生任何错误或加载态
- [ ] A6：给已下载的书加书签、跳转、删除，杀进程重进后书签仍在
