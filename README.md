...
- 刷新按钮限速 2 秒/次（`RefreshThrottle`），窗口内的重复点击静默丢弃、不加提示
- 阅读时隐藏系统状态栏，退出阅读自动恢复
- 书架同步与元信息补全提示移到页面底部
- 设置页新增「开发者」入口，跳转项目 GitHub 主页
- 设置页分区改为描边样式（去掉 surface 填充）
- 设置页拆分出无状态 `SettingsContent`，新增整屏 `@Preview`（见 [§9](#compose-预览preview不是测试但同样不跑设备)）
